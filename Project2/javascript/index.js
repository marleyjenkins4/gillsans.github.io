function PageTransitions() {
    ...
    const getCurrentTransform = () => {
      ...
      currentTransform !== 0
        ? (currentTransform = parseInt(currentTransform[0]))
        : (currentTransform = 0);
  ...
  function Navigation(pagestrs) {
    ...
    const naviInSeq = () => {
      const seq = [animateManuButton, menuIn, menuItemsIn];
      seq.forEach((anim) => {
        anim();
      });
    };
    ...
    const naviOutSeq = () => {
      const seq = [reverseManuButtonAnimation, menuOut];
      seq.forEach((anim) => {
        anim();
      });
    };
    ...
    const menuItemClickSeq = [reverseManuButtonAnimation, menuOut, pagestrs.move];
